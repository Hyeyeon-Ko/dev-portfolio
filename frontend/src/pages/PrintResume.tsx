...
const CAREER_PROJECTS = [
  {
    ...
    highlights: [
      ...
      "배포 직후 발생한 결과지 조회 장애를 로그와 일반 사용자 시점 재현 테스트로 빠르게 원인 범위 특정 및 수정 배포",
      "권한 유형(일반 사용자·관리자·미인증)별 체크리스트를 배포 전 검증 기준으로 도입해 동일 유형 오류 재발 방지",
      "중복 조건 처리와 누락된 예외 케이스를 정리해 유지보수성 개선",
    ],
  ...
  },
  {
    ...
    highlights: [
      ...
      "기안·승인 단계를 상태값 기반으로 세분화하고 상태별 권한, 알림 대상, 화면 동작을 분리 설계",
      "외부 API 연동으로 결재선을 동적 구성하고, 응답 데이터를 내부 포맷으로 정규화해 재사용 가능한 구조로 가공",
      "SMTP 기반 이메일 발송까지 신청→승인→발주 흐름을 하나의 일관된 흐름으로 연결",
      "현업 담당자와 반복적으로 소통하며 요구사항과 예외 케이스를 정리해 API 명세 및 DB 구조 설계",
...
const SIDE_PROJECTS = [
  {
    ...
    highlights: [
      ...
      "AI 도구와 협업하여 Vercel(프론트)·Railway(백엔드·DB) 배포 구성",
    ],
  },
  {
